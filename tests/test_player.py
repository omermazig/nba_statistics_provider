...
class TestStatClass:
    ...
    def test_defense_dashboard(self, player_object):
        if int(player_object.season[:4]) < 2013:
            pytest.skip("Defense Dashboard only works for 2013-2014 season")
    ...
    def test_shot_chart(self, player_object):
        if int(player_object.season[:4]) < 1995:
    ...
    def test_game_logs(self, player_object):
        if int(player_object.season[:4]) < 1980:
            pytest.skip("Shot charts only works for 2013-2014 season")
    ...
    def test_shot_dashboard(self, player_object):
        try:
            ...
            pytest.skip("Shot dashboards only works for 2013-2014 season")
...
class TestRegression:
    ...
    def test_teammate_shooting(self, request, player_object):
        ...
        try:
            _, teammates_shots_after_pass = player_object.get_teammates_efg_percentage_from_passes()
            _, teammates_shots_not_after_pass = player_object._get_teammates_efg_percentage_without_passes()
            teamates_shots = player_object.current_team_object.stats_df['FGA'].item() - player_object.stats_df['FGA'].item()
        except NoStatDashboard:
            pytest.skip("Shot charts only works for 1996-1997 season")
        assert teammates_shots_after_pass + teammates_shots_not_after_pass == teamates_shots
